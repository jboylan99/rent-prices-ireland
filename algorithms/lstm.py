...
df = df.drop(["Statistic", "Property Type", "Location", "Unit", "Per_Person"], axis=1)
...
new_data = pd.DataFrame(index=range(0,len(df)),columns=['Quarter', 'Value'])
for i in range(0,len(data)):
    new_data['Quarter'][i] = data['Quarter'][i]
    new_data['Value'][i] = data['Value'][i]

#setting index
new_data.index = new_data["Quarter"]
new_data.drop('Quarter', axis=1, inplace=True)
